...
export default function LoginGateway({ onLoginSuccess }) {
  ...
  const handleAuthenticationSubmit = async (e) => {
    ...
    // Mock verification for local deployment / Instant boot sequence
    // In production, swap this with an active Axios post request to your Auth API endpoint
    setTimeout(() => {
  ...
  return (
    <div className="h-screen w-screen bg-slate-950 flex items-center justify-center font-sans select-none relative overflow-hidden">
      ...
      <div className="w-full max-w-md p-8 bg-slate-900/80 border border-slate-800 rounded-2xl shadow-2xl backdrop-blur-md z-10 relative">
        ...
        <div className="flex flex-col items-center space-y-3 mb-8 text-center">
          <div className="h-12 w-12 rounded-xl bg-rose-600 flex items-center justify-center font-black text-xl text-white shadow-lg shadow-rose-600/20 animate-pulse">MW</div>
          <div>
            <h1 className="text-md font-black tracking-widest text-white uppercase">MEDICWATCH GATEWAY</h1>
            <p className="text-[11px] font-bold text-slate-500 tracking-wider uppercase mt-1">OPERATIONS COMMAND AUTHENTICATION REQUIRED</p>
          </div>
        </div>
        ...
        <form onSubmit={handleAuthenticationSubmit} className="space-y-4">
          <div>
            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-1.5">OPERATOR ID / EMAIL</label>
            <input
              type="email"
              required
          ...
          <div>
            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-1.5">SECURITY ENCRYPTION PASSWORD</label>
            <input
              type="password"
              required
              placeholder="••••••••••••"
        ...
        <div className="text-center mt-6">
          <p className="text-[9px] font-mono text-slate-600 uppercase tracking-widest">SECURE DATA STREAM // CLEARANCE LEVEL 03</p>
        </div>
      </div>
    </div>
  );
